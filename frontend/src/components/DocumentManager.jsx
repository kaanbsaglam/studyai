...
export default function DocumentManager({ classroomId }) {
  ...
  return (
    <div className="space-y-6">
      {/* Upload Section */}
      <div className="bg-white p-6 rounded-lg shadow-sm border dark:bg-slate-900 dark:border-slate-800 dark:text-slate-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {docs.length === 0 && (
            <div className="col-span-full text-center text-gray-400 dark:text-slate-500 py-10">
                No documents found. Upload one to get started.
            </div>
        )}
        
        {docs.map((doc) => (
          <div key={doc.id} className="bg-white p-4 rounded-xl shadow-sm border flex justify-between items-start group dark:bg-slate-900 dark:border-slate-800">
            <div className="flex items-center gap-3 overflow-hidden">
              <div className="p-3 bg-red-50 text-red-500 rounded-lg shrink-0 dark:bg-red-900/30">
                <FaFilePdf size={20} />
              </div>
              <div className="min-w-0">
                <p className="font-medium truncate text-gray-800 dark:text-slate-100" title={doc.filename}>
                    {doc.filename}
                </p>
            ...
            <button 
              ...
              className="text-gray-300 hover:text-red-500 transition-colors p-2 dark:text-slate-500"
      ...
      {generatedContent && (
        <div className="bg-gray-50 p-4 rounded border mt-4 relative dark:bg-slate-800 dark:border-slate-700">
          <button 
            onClick={() => setGeneratedContent(null)} 
            className="absolute top-2 right-2 text-gray-500 hover:text-red-500 dark:text-slate-400"
          >
            ✕
          </button>
          <h3 className="font-bold capitalize mb-2">{generatedContent.type} Result</h3>
          <pre className="whitespace-pre-wrap text-sm bg-white p-4 rounded border dark:bg-slate-900 dark:border-slate-700">
